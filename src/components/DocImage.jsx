...
const DocImage = ({src, caption, alt, maxWidth = "60%"}) => {
  ...
  return (<figure className="my-8 flex flex-col items-center">
    ...
    <div className={`overflow-hidden rounded-xl border ${tw.border.dim} shadow-md dark:shadow-xl bg-gray-50 ${tw.tcd}`}
         style={{maxWidth: maxWidth, width: "100%",}}
    >
      <img src={src} alt={caption || "Description"} loading="lazy"
           className="w-full h-auto object-cover block transition-transform duration-300 hover:scale-[1.03]"
        // alt='이미지를 찾을 수 없습니다.'
      />
    </div>
    {caption && (<figcaption className={`mt-3 text-sm ${tw.txt.dim} font-medium text-center ${tw.tcd}`}
    >
