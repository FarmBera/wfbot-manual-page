...
export const uiTextsKo = {
  ...
  intro: {
    ...
    </>), imgDesc: `날 보러 와줘서 고맙다냥`, foot: (<NOTE
      color="yellow" icon="info" title="알림" text={<p> 이 홈페이지는 현재 베타 버전입니다. <br/> 오탈자가 있을 수 있으니, 양해 부탁드립니다. </p>}
    />), note: "",
  }, // 2. 주요 기능 섹션
  features: {
    id: "features",
    ...
    items: [{
      ...
      title: "마켓 검색 기능", desc: (<>
        ...
        <br/>
        이 봇을 이용하면 디스코드에서 간단하게 시세를 검색할 수 있어요. 매번 워프레임 마켓 웹사이트에 접속할 필요가 없죠!
        <br/> <br/>
        검색 결과에 마켓으로 이동할 수 있는 링크를 제공하고 있으니, 원한다면 마켓으로 바로 이동할 수도 있죠!
      </>), subDesc: "",
    }, {
      title: "서버 내 파티 모집 시스템", desc: (<>
        매번 파티원들에게 DM 보내기 힘드셨죠? <br/>
        이제 간단한 명령어와 관리 시스템으로 파티 모집을 편리하게 할 수 있어요.
      </>),
    }, {
      title: "서버 내 아이템 거래 시스템", desc: (<>
        간단한 명령어와 관리 시스템으로 길드 내에서 판매/구매 거래를 편리하게 할 수 있어요.<br/> <br/>
        또한 마켓 가격을 기반, 자동 가격 책정 기능이 내장되어있어, 초보자 분들도 시세 걱정 없이 거래 게시글을 작성할 수 있어요.
      </>),
    },],
  }, // 3. 명령어 섹션
  manual: {
    id: "manual", title: "3. 워프레임 컨텐츠 명령어", mainTitle: "모든 명령어 목록", list: [{
      ...
      foot: (<>
        ...
        <ol className={htags.olTag}>
          ...
          <li><strong>item_name</strong> 부분에 검색하고자 하는 아이템을 입력해주세요.</li>
        </ol>
        <DocImage src={`${IMG_PATH}market-search-searching${IMG_EXT}`}/>
        <p className={htags.pTag}> 자동완성 기능을 지원하고 있어, 아이템을 검색해 볼 수 있어요. </p>
        <NOTE color="yellow" icon="alert" title="알림"
              text={<>일부 변역이 되지 않은 아이템이 있을 수 있습니다. <br/> 검색이 되지 않는 항목이 있다면 <ContactMe/></>}
        />
      </>),
    }, {
  ...
  terms: {
    id: "tos", title: "이용 약관", desc: (<>
      ...
      <p>본 약관은 {COMMON.name}(이하 "서비스") 봇이 제공하는 모든 기능과 콘텐츠, 웹사이트를 이용함에 있어, 서비스를 제공해드리는
        {COMMON.company}(이하 "운영자")과 이용자 간의 권리, 의무 및 책임 사항을 규정함을 목적으로 합니다.
      </p>
      ...
      <ul className={htags.ulTag}>
        ...
        <li>약관이 변경될 경우, 디스코드 봇 공지사항 또는 지원 서버 등을 통해 사전 공지합니다.</li>
        <li>이용자가 변경된 약관 공지 후에도 서비스를 계속 이용하는 경우, 변경 사항에 동의한 것으로 간주합니다. 동의하지 않을 경우 서비스 이용을 중단할 수 있습니다.</li>
      </ul>
      ...
      <ul className={htags.ulTag}>
        ...
        <li>서비스의 변경 또는 중단으로 인해 발생하는 이용자의 손해에 대해서는 책임을 지지 않고 별도의 보상을 하지 않습니다.</li>
      </ul>
      <H2 text="이용자의 의무"/>
      <ul className={htags.ulTag}>
        <li>이용자는 디스코드의 자체 이용약관(Terms of Service) 및 커뮤니티 가이드라인을 준수해야 합니다.</li>
        <li>이용자는 서비스를 오직 합법적인 목적으로만 사용해야 하며, 다음 각 호의 행위를 금지합니다.</li>
        <ul className={htags.ulTag}>
        ...
        <li>서비스 이용 중 버그나 보안 취약점을 발견할 경우, 이용자는 즉시 운영자에게 제보해야 하며 이를 악용해서는 안됩니다.</li>
        <li>위 의무를 위반할 경우, 운영자는 사전 통보 없이 해당 이용자의 서비스 이용을 제한하거나 영구 차단할 수 있습니다.</li>
      </ul>
      <H2 text="개인정보 보호"/>
      ...
      <ul className={htags.ulTag}>
        ...
        <li>운영자는 이용자의 귀책 사유로 인한 서비스 이용의 장애나 데이터 손실에 대해 책임을 지지 않습니다.</li>
        <li>운영자는 이용자가 서비스를 이용하여 기대하는 수익을 상실하거나, 서비스를 통해 얻은 자료로 인해 발생한 손해에 대해 운영자의 고의 또는 중대 과실이 없는 한 책임을 지지 않습니다.</li>
        <li>운영자는 이용자간 서비스를 매개로 하여 발생한 분쟁에 개입할 의무가 없으며, 이로 인한 손해를 배상할 책임도 없습니다.</li>
        <li>운영자가 제공하는 거래 관련 기능은 이용자 간의 편의를 위한 보조 도구일 뿐이며, 실제 거래의 이행 및 결과에 대한 책임은 거래 당사자인 이용자에게 있습니다.</li>
        <ul className={htags.ulTag}>
          <li>서비스를 통해 이루어지는 이용자 간의 거래는 전적으로 이용자의 판단과 책임 하에 이루어집니다.</li>
          <li>거래 기능을 이용함에 있어 발생할 수 있는 게임 내 재화의 손실, 사기 피해 및 기타 금전적 손해에 대해 스스로 책임을 부담하며,
            운영자는 이에 대해 고의 또는 중대한 과실이 없는 한 어떠한 책임도 지지 않습니다.
          </li>
          <li>이용자가 게임사의 운영 정책 미숙지로 발생한 계정 제재 및 아이템 회수에 대해서 운영자는 책임을 지지 않습니다.</li>
        </ul>
      </ul>
      <H2 text="저작권"/>
      ...
      <H2 text="링크"/>
      <P
        text="서비스 내에 제3자가 제공하는 외부 웹사이트 및 자원에 대한 링크가 포함되는 경우, 이러한 링크는 이용자의 편의를 위해 제공되는 것입니다. 운영자는 해당 외부 사이트의 콘텐츠나 정책을 통제하지 않으며, 이용자가 이를 이용함으로써 발생하는 문제에 대해 책임을 지지 않습니다."/>
  ...
  privacy: {
    id: "privacy", title: "개인정보 처리방침", desc: (<>
      ...
      <ul className={htags.ulTag}>
        <li><strong>권리 행사 방법</strong>: 운영자 이메일 또는 공식 지원 서버를 통해 서면, 채팅 등으로 요청</li>
      ...
      <P
        text="본 개인정보 처리방침은 법령, 정책 또는 보안 기술의 변경에 따라 내용이 추가, 삭제 및 수정될 수 있습니다. 변경 사항이 있을 경우 시행 7일 전부터 디스코드 봇 공지사항 또는 공식 지원 서버를 통해 공지할 것입니다."/>
      <ul className={htags.ulTag}>
        <li>공고 일자: 2025년 12월 4일</li>
        <li>시행 일자: 2025년 12월 10일</li>
        <li>개정 일자: 2025년 12월 24일</li>
  ...
  legal: {
    id: "legal", title: "저작권 및 법적 고지", desc: (<>
      ...
      <p>서비스 이용자는 운영자의 명시적인 서면 동의 없이 본 서비스의 코드를 복제, 수정, 배포하거나 2차 저작물을 생성할 수 없습니다.
