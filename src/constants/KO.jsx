...
const IMG_EXT = ".webp";
const ordis_msg = getOrdisMsg()

export const uiTextsKo = {
  // common UI
  docTitle: <span>클랜 봇 홈페이지</span>,
  ...
  features: {
    ...
    head: (<P text="실시간 워프레임 컨텐츠를 제공해주고, 추가 편의 기능을 제공합니다."/>),
    items: [{
      title: "실시간 워프레임 컨텐츠 확인", desc: "명령어를 이용하여 워프레임 내 컨텐츠를 확인할 수 있어요.", subDesc: "(바로키티어 등장 상태, 출격, 침공, 얼럿 미션 등등)",
    }, {
      title: "실시간 컨텐츠 알림", desc: (<>
        새로운 컨텐츠가 업데이트 될 때 마다 실시간으로 메시지를 보내 줍니다.
      </>),
    }, {
      title: "마켓 검색 기능", desc: (<>
        매번 마켓에 접속해서 시세를 검색하신다면?<br/><br/>
        이 봇을 이용하면 디스코드에서 간단하게 시세를 검색할 수 있어요. 매번 워프레임 마켓 웹사이트에 접속할 필요가 없죠!<br/>
      </>), subDesc: "검색 결과에 마켓으로 이동할 수 있는 링크를 제공하고 있으니, 원한다면 마켓으로 바로 이동할 수도 있죠!",
    }, {
      title: "서버 내 파티 모집 시스템", desc: (<>
        매번 파티원들에게 DM 보내기 힘드셨죠? <br/><br/>
        이제 간단한 명령어와 관리 시스템으로 파티 모집을 편리하게 할 수 있어요.
      </>),
    }, {
      title: "서버 내 아이템 거래 시스템", desc: (<>
        간단한 명령어와 관리 시스템으로 <br/>
        길드 내에서 아이템 판매/구매를 편리하게 할 수 있어요.<br/><br/>
    ...
    id: "manual", title: "3. 사용 가능한 명령어", mainTitle: "모든 명령어 목록", list: [{
      ...
      id: SUB.MARKET_SEARCH, title: "마켓검색", desc: (<>
        ...
        <NOTE color="yellow" icon="alert" title="알림"
              text={<>검색 결과에 나오지 않는 아이템이 있다면 <ContactMe/></>}
        />
      </>),
    }, {
    ...
    id: "party", title: "4. 파티 모집 기능", mainTitle: "파티 모집 명령어", list: [// 파티 기능 개요
      {
        ...
        id: "party-edit-member", title: "(4-1) 인원 수정", foot: (<>
          ...
          <ul className={htags.ulTag}>
            <li><strong>파티장(본인)을 포함</strong>한 인원 수 입니다.</li>
            <li><strong>2 ~ 20명</strong> 사이로 설정 가능합니다.</li>
            <li> 숫자가 아닌 값을 입력할 시, <strong>기본값 4명</strong>이 자동 설정됩니다. <br/> (워프레임 최대 파티원 수 4명)</li>
          </ul>
        ...
        id: "party-edit-article", title: "(4-2) 게시글 수정", desc: "잘못 입력한 내용을 수정하기 위한 기능입니다.", foot: (<>
          ...
          <P text='글 수정 버튼을 누르면 다음과 같은 창이 표시됩니다.'/>
          <DocImage src={`${IMG_PATH}party-mod1${IMG_EXT}`} caption="글 수정 팝업창"/>
          <p className={htags.pTag}>
            수정하고자 하는 항목을 수정 후 <strong className={`${tw.txt.sky} ${tw.bold.eb}`}>전송</strong> 버튼을 누르면 항목이 수정됩니다.
          </p>
        ...
        id: "party-change", title: "(4-4) 모집 상태 전환", desc: "모집 글의 모집 상태를 전환할 수 있습니다.", foot: (<>
          ...
          <DocImage src={`${IMG_PATH}party-toggle${IMG_EXT}`} caption="모집 완료 상태"/>
          <p className={htags.pTag}>모집 완료 상태에서는</p>
          <ul className={htags.ulTag}>
            <li><strong>파티원 참여가 불가</strong> 합니다.</li>
            <li><strong>인원 수정이 불가</strong> 합니다</li>
          </ul>
          <p className={htags.pTag}>
        ...
        id: "party-call", title: "(4-5) 파티원 호출", desc: "파티원 분들에게 호출 알림을 보냅니다", foot: (<>
          ...
          <p className={htags.pTag}>사용 예: 출발 알림, 모든 유저들에게 특별 공지 등</p>
          <DocImage src={`${IMG_PATH}party-call-result${IMG_EXT}`} maxWidth="75%" caption="파티원 호출 메시지 예"/>
          <p className={htags.pTag}>
            과도한 사용 방지를 위해 한번 사용 시 15분 쿨타임이 적용됩니다.<br/>
            해당 기능을 악용 할 경우 (예: 알림 테러) 봇이 사용 패턴을 분석하여 블랙리스트에 자동으로 등록 및 제재를 받을 수 있습니다. <br/>
            또한 사용자에게 지속적으로 부정 사용 건의를 받고 있습니다. 부정 사용 적발 시 제재를 받을 수 있습니다.
          </p>
    ...
    id: "trade", title: "5. 거래 기능", mainTitle: "거래 명령어", list: [// trade warnings
      {
        ...
        foot: (<div>
          ...
          <ul className={htags.ulTag}>
            ...
            <li>
              거래하는 아이템에 상응하는 재화가 반드시 <span className="font-bold">[플레티넘]</span>일 필요는 없으며, 이것은 판매자 분들의 결정에 전적으로 달려있습니다.
            </li>
            <ul className={htags.ulTag}>
              <li>
              ...
              <li>
                <A href="https://www.warframe.com/ko/eula#virtualGoods" text="워프레임 소프트웨어 사용권 계약(EULA)"/>{" "}
  ...
  terms: {
    id: "tos", title: "이용 약관", desc: (<>
      ...
      <p>본 약관은 {COMMON.name}(이하 "서비스") 봇이 제공하는 모든 기능과 콘텐츠, 웹사이트를 이용함에 있어, 서비스를 제공해드리는
        {COMMON.company}(이하 "운영자")과 사용자 간의 권리, 의무 및 책임 사항을 규정함을 목적으로 합니다.
      </p>
      <P text="사용자가 봇을 서버에 추가하거나 기능을 사용함으로써 본 약관에 동의하는 것으로 간주합니다."/>
      <H2 text="약관의 효력과 변경"/>
      <ul className={htags.ulTag}>
        <li>본 약관은 서비스를 이용하는 모든 사용자에게 적용됩니다.</li>
        <li>운영자는 필요한 경우 관련 법령을 위배하지 않는 범위 내에서 약관을 개정할 수 있습니다.</li>
        <li>약관이 변경될 경우, 디스코드 봇 공지사항 또는 디스코드 서버 등을 통해 사전 공지합니다.</li>
        <li>사용자가 변경된 약관 공지 후에도 서비스를 계속 이용하는 경우, 변경 사항에 동의한 것으로 간주합니다. 동의하지 않을 경우 서비스 이용을 중단할 수 있습니다.</li>
      </ul>
      <H2 text="서비스의 제공 및 변경"/>
      <ul className={htags.ulTag}>
        <li>운영자는 디스코드 플랫폼 내에서 사용자 편의를 위한 다양한 기능을 제공합니다.</li>
        <li>운영자는 유지/보수, 디스코드 API 정책 변경, 천재지변 등 불가피한 사유가 발생할 경우 서비스의 전부 또는 일부를 변경하거나 중단할 수 있습니다.</li>
        <li>서비스의 변경 또는 중단으로 인해 발생하는 사용자의 손해에 대해서는 책임을 지지 않고 별도의 보상을 하지 않습니다.</li>
      </ul>
      <H2 text="사용자의 의무"/>
      <ul className={htags.ulTag}>
        ...
        <li>서비스 이용 중 버그나 보안 취약점을 발견할 경우, 사용자는 즉시 운영자에게 제보해야 하며 이를 악용해서는 안됩니다.</li>
        <li>위 의무를 위반할 경우, 운영자는 사전 통보 없이 해당 사용자의 서비스 이용을 제한하거나 영구 차단할 수 있습니다.</li>
      </ul>
      <H2 text="개인정보 보호"/>
      ...
      <p className={htags.pTag}> 상세한 내용은 별도로 고지된 <strong>개인정보 처리방침</strong>을 따릅니다. <br/>
        좌측 사이드바에서 개인정보 처리방침을 클릭하여 확인해주세요. (모바일은 우측 상단 메뉴 버튼을 통해 이동할 수 있습니다.)
      </p>
      <H2 text="책임의 제한 (면책)"/>
      <ul className={htags.ulTag}>
        <li>운영자는 천재지변, 게임사의 서버 점검, 디스코드 플랫폼의 장애, 통신망의 장애, 또는 이에 준하는 불가항력으로 인해 서비스를 제공할 수 없는 경우에 대해 책임을 지지 않습니다.</li>
        <li>운영자는 사용자의 귀책 사유로 인한 서비스 이용의 장애나 데이터 손실에 대해 책임을 지지 않습니다.</li>
        <li>운영자는 사용자가 서비스를 이용하여 기대하는 수익을 상실하거나, 서비스를 통해 얻은 자료로 인해 발생한 손해에 대해 운영자의 고의 또는 중대 과실이 없는 한 책임을 지지 않습니다.</li>
        <li>운영자는 사용자간 서비스를 매개로 하여 발생한 분쟁에 개입할 의무가 없으며, 이로 인한 손해를 배상할 책임도 없습니다.</li>
        <li>운영자가 제공하는 거래 관련 기능은 사용자 간의 편의를 위한 보조 도구일 뿐이며, 실제 거래의 이행 및 결과에 대한 책임은 거래 당사자인 사용자에게 있습니다.</li>
        <ul className={htags.ulTag}>
          <li>서비스를 통해 이루어지는 사용자 간의 거래는 전적으로 사용자의 판단과 책임 하에 이루어집니다.</li>
          <li>거래 기능을 이용함에 있어 발생할 수 있는 게임 내 재화의 손실, 사기 피해 및 기타 금전적 손해에 대해 스스로 책임을 부담하며,
            운영자는 이에 대해 고의 또는 중대한 과실이 없는 한 어떠한 책임도 지지 않습니다.
          </li>
          <li>사용자가 게임사의 운영 정책 미숙지로 발생한 계정 제재 및 아이템 회수에 대해서 운영자는 책임을 지지 않습니다.</li>
        </ul>
      </ul>
      <H2 text="저작권"/>
      ...
      <H2 text="링크"/>
      <P
        text="서비스 내에 제3자가 제공하는 외부 웹사이트 및 자원에 대한 링크가 포함되는 경우, 이러한 링크는 사용자의 편의를 위해 제공되는 것입니다. 운영자는 해당 외부 사이트의 콘텐츠나 정책을 통제하지 않으며, 사용자가 이를 이용함으로써 발생하는 문제에 대해 책임을 지지 않습니다."/>
      ...
      <P text="변경 사항이 있을 경우 시행 7일 전부터 디스코드 봇 공지사항 또는 디스코드 서버를 통해 공지할 것입니다."/>
      <ul className={htags.ulTag}>
  ...
  privacy: {
    id: "privacy", title: "개인정보 처리방침", desc: (<>
      <p>
        ...
        (이하 “운영자”)은 사용자의 개인정보를 소중하게 생각하며, 사용자의
      ...
      <ul className={htags.ulTag}>
        <li><strong>사용자 식별 정보</strong>: 디스코드 사용자 ID, 닉네임</li>
        <li><strong>이용 환경 정보</strong>: 명령어가 사용된 서버(Guild)의 이름 및 ID, 채널 이름 및 ID</li>
        <li><strong>서비스 이용 기록</strong>: 사용한 명령어, 팝업창(Modal) 등을 통해 입력된 데이터, 오류 발생 로그</li>
      </ul>
      ...
      <ol className={htags.olTag}>
        ...
        <li><strong>서비스 개선</strong>: 이용 통계 분석을 통한 선호 기능을 파악하여 기존 기능을 개선, 또는 신규 기능 개발 자료로 활용</li>
        <li><strong>부정 사용자 식별</strong>: 이용약관 위반 등의 행위로 인하여 봇 이용이 정지된 사용자 식별</li>
      </ol>

      <H2 text="개인정보 보유 기간"/>
      <P text="사용자의 개인정보는 원칙적으로 개인정보의 수집 및 이용 목적이 달성되면 지체 없이 파기합니다. 단, 다음의 정보는 아래의 사유로 명시한 기간 동안 보존합니다."/>
      <ul className={htags.ulTag}>
        <li><strong>보존 항목</strong>: 서비스 이용 기록 및 로그 데이터</li>
        <li><strong>보존 기간</strong>: 수집일로부터 최대 1년</li>
      ...
      <ul className={htags.ulTag}>
        <li><strong>절차</strong>: 보존 기간이 경과하거나 처리 목적이 달성된 개인정보는 내부 방침에 따라 안전하게 파기됩니다.</li>
        <li><strong>전자적 파일 형태</strong>: 복구 및 재생할 수 없는 기술적 방법을 사용하여 영구 삭제합니다.</li>
      </ul>
      ...
      <H2 text="사용자의 권리와 행사 방법"/>
      <P text="사용자는 언제든지 등록되어 있는 자신의 개인정보를 조회하거나 삭제를 요청할 수 있습니다."/>
      <ul className={htags.ulTag}>
        <li><strong>권리 행사 방법</strong>: 운영자 이메일 또는 디스코드 서버를 통해 서면, 채팅 등으로 요청</li>
        <li><strong>요청 가능 사항</strong>: 개인정보 열람, 정정, 삭제, 처리 정지 요구</li>
      </ul>

      <H2 text="개인정보 안전성 확보 조치"/>
      <P text="운영자는 개인정보의 안전성 확보를 위해 다음과 같은 조치를 취하고 있습니다."/>
      <ol className={htags.ulTag}>
        <li><strong>관리적 조치</strong>: 개인정보에 접근할 수 있는 인원을 운영자로 최소화하고 있습니다.</li>
        <li><strong>기술적 조치</strong>:</li>
        <ul className={htags.ulTag}>
          <li>저장된 데이터는 하드웨어 레벨의 암호화 기술이 적용된 저장 장치에 보관됩니다.</li>
          <li>외부 전송 시 데이터 암호화 및 암호화된 통신 프로토콜을 사용합니다.</li>
          <li>정기적인 소프트웨어 업데이트를 통해 해킹이나 바이러스로부터 시스템을 보호합니다.</li>
        </ul>
      </ol>
      ...
      <ul className={htags.ulTag}>
        ...
        <li>이메일: <A href={`mailto:${process.env.REACT_APP_CONTACT_EMAIL}`} text={process.env.REACT_APP_CONTACT_EMAIL}/>
        </li>
      ...
      <P
        text="본 개인정보 처리방침은 법령, 정책 또는 보안 기술의 변경에 따라 내용이 추가, 삭제 및 수정될 수 있습니다. 변경 사항이 있을 경우 시행 7일 전부터 봇 공지사항 명령어 및 또는 디스코드 서버를 통해 공지할 것입니다."/>
      <ul className={htags.ulTag}>
        <li>공고 일자: 2025년 12월 4일</li>
        <li>시행 일자: 2025년 12월 10일</li>
        <li>개정 일자: 2025년 12월 24일</li>
  ...
  legal: {
    id: "legal", title: "저작권 및 법적 고지", desc: (<>
      ...
      <p>{COMMON.name}(이하 서비스) 의 개발 및 운영과 관련된 독자적인 소스 코드, 데이터베이스 구조, 웹사이트 디자인 등에 대한 저작권은
        개발자 {COMMON.company}(이하 운영자)에게 귀속됩니다.
      </p>
      <p>서비스 사용자는 운영자의 명시적인 서면 동의 없이 본 서비스의 코드를 복제, 수정, 배포하거나 2차 저작물을 생성할 수 없습니다.
        (단, 오픈 소스로 공개된 부분은 해당 라이선스를 따릅니다.)
      </p>
      <H3 text="제3자 콘텐츠 및 상표권"/>
      <p>본 서비스 내에서 제공되는 게임 <strong>'워프레임 (Warframe)'</strong> 과 관련된 모든 데이터, 예술 저작물, 로고, 아이콘, 텍스트, 고유명사 등의
        지적재산권은 <strong>Digital Extremes Ltd.</strong>에 있습니다.
      </p>
      <p><strong>Warframe©</strong>은 Digital Extremes Ltd.의 등록 상표입니다.
        본 서비스에서 사용된 워프레임(Warframe) 관련 이미지 및 컨텐츠는 정보 전달을 목적으로 사용되었으며, 저작권자의 권리를 침해할 의도가 없습니다.
      </p>
  ...
  contact: {
    id: "contact", title: "도움이 필요하신가요?", foot: (<>
      ...
      <NOTE color='orange' icon='alert' title="개인 DM으로는 문의를 받지 않습니다."
            text={<>
              개인 DM은 일상 메시지와 섞여 확인이 어렵거나, 답변이 매우 늦어질 수 있습니다.<br/>
              빠르고 정확한 답변을 위해, <strong>각종 문의 & 버그 제보는 상기 연락수단을 이용</strong>하여 문의 바랍니다.
            </>}
      />
