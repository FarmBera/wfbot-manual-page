...
const SIDEBAR_WIDTH: number = 410; // Sidebar fixed width (px)
const CONTENT_WIDTH: number = 860; // Content fixed width (px)
const TOTAL_WIDTH_TRIGGER: number = SIDEBAR_WIDTH + CONTENT_WIDTH; // trigger mobile mode if window is smaller than this

// const MOBILE_UI_SIZE: number = 768; // Tailwind CSS mobile UI size breakpoint
...
const UserManual: React.FC = () => {
    ...
    const [isMobile, setIsMobile] = useState<boolean>(() => {
        if (typeof window !== "undefined") return window.innerWidth < TOTAL_WIDTH_TRIGGER;
        return false;
    });
    ...
    const sidebarRef = useRef<HTMLElement>(null); // ref for access 'sidebar navigation DOM'
    ...
    const toggleExpand = (sectionId: string) => {
        setExpandedIds((prev) => prev.includes(sectionId) ? prev.filter((id) => id !== sectionId) // collapse: remove if exists
            : [...prev, sectionId] // expand: add if not exists
        );
    };

    // onclick sidebar item (move + toggle)
    const handleItemClick = (e: React.MouseEvent, section: Section) => {
        e.preventDefault();
    ...
    // detect scroll & auto sidebar expand (scroll event)
    useEffect(() => {
        const handleScroll = () => {
            ...
            let currentItem: Section | SubSection | null = null;
    ...
    // on initial component load, check for 'id' query parameter (Initial ID check)
    useEffect(() => {
    ...
    useEffect(() => {
        if (activeSection && sidebarRef.current) {
            ...
            const activeElement = sidebarRef.current.querySelector(`[data-section-id="${activeSection}"]`);
            if (activeElement) {
    ...
    // close lang dropdown menu onclick other area (Click outside lang menu)
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (isLangMenuOpen && !(event.target as Element).closest(".lang-dropdown")) {
                setIsLangMenuOpen(false);
            }
        };
    ...
    // [resize handler] resize event listener (detect screen size changes)
    useEffect(() => {
        const handleResize = () => setIsMobile(window.innerWidth < TOTAL_WIDTH_TRIGGER); // or MOBILE_UI_SIZE based on logic
    ...
    useEffect(() => {
        const handleResize = () => setIsMobile(window.innerWidth < TOTAL_WIDTH_TRIGGER);
        window.addEventListener("resize", handleResize);
        return () => window.removeEventListener("resize", handleResize);
    }, []);
    ...
    return (<div className={`flex h-screen font-sans overflow-hidden break-keep ${tw.tcd}`}>
            ...
            <div className="flex items-center space-x-2">
                ...
                <div className="relative lang-dropdown">
                    ...
                    {isLangMenuOpen && (<div
                      ...
                      ${lang === l.code ? `${tw.txt.theme} font-lg ${tw.tcd}` : `${tw.txt.normal} ${tw.tc} ${tw.d2}`}
                    `}
                        >
                            {l.label}
        ...
        <aside
          ...
          ${isMobile ? `fixed top-0 left-0 pt-14 shadow-2xl ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}` : `relative translate-x-0 pt-0`}
        `}>
            {/* desktop header */}
            {!isMobile && (<div
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <div>
                        <h1 className={`font-bold text-lg ${tw.txt.bright} ${tw.tcd}`}>{uiText.docTitle}</h1>
                        <p className={`text-xs ${tw.txt.dim} ${tw.tcd}`}>{APP_INFO.version}</p>
                    </div>
                </div>
            ...
            {/*<div className="p-4 pb-0">
                <div className="relative">
                    <Search size={16} className={`absolute left-3 top-2.5 ${tw.txt.dark}`}/>
                    <input type="text" disabled={true} placeholder={uiText.searchPlaceholder}
            ...
            <nav ref={sidebarRef} className="flex-1 overflow-y-auto p-4 space-y-1">
                ...
                `}>
                        ...
                        >
                          ...
                          ${activeSection === sub.id ? `${tw.txt.theme} font-bold ${tw.bg.theme}` : `${tw.txt.dim} hover:bg-gray-300 dark:hover:bg-gray-600 ${tw.border.bright}`}
                        `}>
                                {sub.title}
                            </button>))}
                        </div>)}
            ...
            >
                ...
                >
                    <div className={`flex items-center space-x-3 mb-6 pb-4 border-b ${tw.border.gray} ${tw.tcd}`}>
                        ...
                        <h2 className={`text-2xl font-bold ${tw.txt.light} ${tw.tcd}`}>{section.title}</h2>
                    </div>
                ...
                {/* footer msg */}
                <div className={`text-center py-10 ${tw.txt.dark} text-sm ${tw.tcd}`}>{uiText.footerMsg}</div>
            </div>
        </main>
    </div>);
};
